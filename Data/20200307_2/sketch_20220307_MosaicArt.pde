...
String mainImage = "R_Ambition_Love_Life_5_8000.JPG";
//the name of the main image.
//元の画像

String imageName = "R_Ambition_Love_Life_5_8000.JPG";
...
void setup(){
  ...
  */
  ...
  //fileName = "R_Ambition_Love_Life_5";  //camelRule
  //noLoop();
}
...
void draw(){
  ...
  for (int i = 0; i < pics.length; i++){
    ...
    color avColor = averageColor(pics[i]);  //画像の平均の色
  ...
  for (int u = 0; u < width; u += pixelSize){
    for (int v = 0; v < height; v += pixelSize) {
      ...
      color sampleColor = averageColor(mosaicPixel);  //元画像のモザイク一つの平均RGB
